...
//			_self.currnetMonth = (date.getFullYear()) + "." + (_self.tom(date.getMonth() + 1) + "." + date.getDate());
...
//					date_frame: date.getFullYear() + "." + (_self.tom(date.getMonth() + 1)) + "." + (_self.tod(date.getDate())),
					date_frame: (_self.tom(date.getMonth() + 1)) + "." + (_self.tod(date.getDate())),//去掉年，
				});
			}
			_self.currnetMonth = (tmpInfo[0].date_frame + "-" + tmpInfo[6].date_frame);//修改为周历的月+日
			// 初始化成功执行一次
			...
			console.log(_self.currnetMonth)
			//渲染模板
