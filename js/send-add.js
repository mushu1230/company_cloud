...
function renderLeft(data, dom) {
    ...
    if (Object.prototype.toString.call(data) === '[object Array]') {
        for (var i = 0; i < data.length; i++) {
            ...
            // 渲染dom
            item.innerHTML = '<div class="weui-cell to__item " id="group_' + data[i].group + '">' + arrow + '<span class="member" onclick="nodeClick(this)"><input id="input'+ data[i].group +'" type="checkbox"  ' + checked + '" name="cName"  value="' + data[i].name + '" onclick="checkboxClick(this)" />' + prefix + '<div class="text"><div class="to__name">' + data[i].name + '</div><div class="to__post">' +data[i].post + '</div></div>' + numberDom + '<div class="to__parentName">' + parentName + '</div><span class="icon__round ' + iconClass + '"></span></span></div>';
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (item.children.length > 0 && item.last != true) {
            ...
            if ((item.last == true || oldSearch) && item.active == true) {
                ...
                div.innerHTML = '<span class="unread" style="display:block">未读</span><span class="to__prefix"><img src="img/header-icon.png"></img></span><span class="to__name render" id="render' + item.group + '" >' + item.name + '</span><div class="to__parentName" style="display:none"></div><span id="to__close" class="to__close" onclick="nodeCencel(this)"><i></i></span>';
