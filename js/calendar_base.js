...
console.log(dateObj)
			...
			self.nowMonth = parseInt(dateObj.month);
			console.log(self.nowMonth)
			

			self.nowDay = parseInt(dateObj.day);
		...
		_render: function(dataObj, callback) {
			...
			setTimeout(function() {
				self.options.dataRequest(dateStr, function(data) {
					...
					console.log(dataObj.tmpInfo)
					for(var j = 0; j < res.length; j++) {
						for(var i = 0; i < dataObj.tmpInfo.length; i++) {
							if(res[j].date == dataObj.tmpInfo[i].date) {
								dataObj.tmpInfo[i].isSelected = "1";
								//考勤异常，正常
								if(res[j].value.indexOf("正常")) {
									dataObj.tmpInfo[i].zc = "yc";
								}
								if(res[j].value.indexOf("异常")) {
									dataObj.tmpInfo[i].yc = "zc";
								}
								$(".tipnext").find(".yc").hide();
//								console.log($(".tipnext"))
								$(".tipnext").find(".zc").hide();
			...
			for(var i = 0, len = dataObj.tmpInfo.length; i < len; i++) {
				...
				if(!templData) {
					...
					if(value.date == curr) {
						//今天
						if(value.isSelected) {
							template = '<div class="em-calendar-item em-calendar-active  isforbid{{isforbid}}" date="{{date}}"><span class="day">{{day}}</span><p class="lunar">今天</p><span class="{{yc}} {{zc}}"></span><span class="dot"></span></div>';
						} else {
							template = '<div class="em-calendar-item em-calendar-active  isforbid{{isforbid}}" date="{{date}}"><span class="day">{{day}}</span><p class="lunar">今天</p><span class="dot"></span></div>';
						}
					} else {
						if(value.isSelected) {
							// 个性化和业务贴近
							template = '<div class="em-calendar-item isforbid{{isforbid}} tip{{tip}}" date="{{date}}" ><span class="day">{{day}}</span><p class="lunar">{{lunar}}</p><span class="{{yc}} {{zc}}"></span><span class="dot"></span></div>';
						} else {
							template = '<div class="em-calendar-item isforbid{{isforbid}} tip{{tip}}" date="{{date}}"><span class="day">{{day}}</span><p class="lunar">{{lunar}}</p><span class="dot"></span></div>';
						}
					}
